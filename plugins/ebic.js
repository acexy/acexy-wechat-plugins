...
program.command("ebic bindSubAppid <env> <subMchId> <subAppId>", '为子商户号绑定subAppId \n 例如: ebic bindSubAppid test 3333333 wxwxwxwxx' +
    ' \n 参数说明 \n\t env[执行环境]: test = 测试环境; prd-online = 生产线上环境; prd-offline = 生产线下环境', async function (env, subMchId, subAppId) {
...
program.command("ebic addPayUrl <env> <subMchId> <payUrl>", '为子商户号绑定subAppId \n 例如: ebic bindSubAppid test 3333333 https://pay.com/' +
    ' \n 参数说明 \n\t env[执行环境]: test = 测试环境; prd-online = 生产线上环境; prd-offline = 生产线下环境', async function (env, subMchId, payUrl) {
...
const doRequest = async (wxpay, reqData) => {
    ...
    if (response.xml.return_code == 'SUCCESS' && response.xml.result_code == 'SUCCESS') {
        ...
        return '操作失败: ' + response.xml.err_code_des;
    }
}
